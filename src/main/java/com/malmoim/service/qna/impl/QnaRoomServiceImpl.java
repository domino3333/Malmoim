...
public class QnaRoomServiceImpl implements QnaRoomService {
    ...
    public void createQnaRoom(CreateQnaRoomRequest dto, String hostEmail) {
        ...
        if (Boolean.TRUE.equals(dto.getIsPrivate())) {
    ...
    public QnaPhaseResponse startVotingPhase(String hostEmail, long durationSeconds, long roomNo) {
        ...
        if (qnaRoom == null || qnaRoom.getStatus() != QnaPhase.QUESTION_CLOSED) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "현재 질문 종료 페이즈가 아니므로 투표를 시작할 수 없습니다.");
        }
    ...
    public QnaRoomInfoResponse getParticipantQnaRoom(Long participantNo, Long roomNo) {
        ...
        Integer participantExists = participantMapper.existsByParticipantNoAndRoomNo(participantNo, roomNo);
